...
<div class="container-fluid">
    <div class="row ">
        <div class="col-md-12">
            <div class="card">
            <div class="card-header">{{ __('Cor Atualizar') }} <a class="float-right"  href="{{ route('cor.index', $cor->Produto_id) }}">Voltar <i class="fas fa-hand-point-left"></i></a> </div>
                ...
                {{-- Formulário --}}
                <form role="form" action="{{ route('cor.update', $cor->id) }}" method="POST" enctype="multipart/form-data"> 
                  {{ csrf_field() }}
                  {{ method_field('PUT') }}
                  
                  @include('painel/Cor/form')  
