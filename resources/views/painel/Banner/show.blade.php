                ...
                <div class="card-body">
                    ...
                    <table id="example2" class="table table-bordered table-striped">
                        ...
                        <tr>
                          ...
                          <th>Data Postagem</th>
                          <th>Status</th>
                          <th ><a href="{{ route('banner.create')}}" class="btn btn-lg btn-outline-primary float-right"> <i class="fas fa-plus"></i> Adicionar</a>
                          </th>
                        </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr role="row" class="odd">
                                <td class="sorting_1" tabindex="0">{{ $banner->id}}</td>
                                <td>{{ $banner->Banner_title}}</td>
                                <td>{{ $banner->Banner_data}}</td>
                                <td>
                                ...
                                @endif</td>
                                <td width="150px">
                                <form action="{{ route('banner.destroy', $banner->id )}}" method="Post">  {{ csrf_field() }} {{ method_field('DELETE')}}
                                    <button class="btn btn-outline-primary float-left" type="submit" onclick="return confirm('Tem certeza que deseja deletar?')">
                                        <i class="fas fa-trash-alt"></i>
                                    </button>
                                </form>
                                <a href="{{ route('banner.edit', $banner->id)}}" class="btn btn-outline-primary"> <i class="fas fa-pen-alt"></i></a>
                                </td>
                              </tr>
                             @empty
