    ...
    <div class="row ">
        <div class="col-md-12">
            <div class="card">
                 ...
                 <div class="card-header">{{ __('Cor - ').$product->name }}  <a class="float-right"  href="{{ route('selo.index', $product->id) }}">Voltar <i class="fas fa-hand-point-left"></i></a> </div>
                ...
                <div class="card-body">
                    ...
                    <table id="example2" class="table table-bordered table-striped">
                        ...
                        <tr>
                          ...
                          <th>Hexa</th>
                          <th>Status</th>
                          <th ><a href="{{ route('selo.create', $product->id)}}" class="btn btn-lg btn-outline-primary "> <i class="fas fa-plus"></i> Adicionar</a>
                            
                        </th>
                        </tr>
                        </thead>
                        <tbody>
                            @forelse($selos as $item)
                            <tr role="row" class="odd">
                                ...
                                @endif</td>
                                <td width="150px">
                                <form action="{{ route('selo.destroy', $item->id )}}" method="Post">  {{ csrf_field() }} {{ method_field('DELETE')}}
                                    <button class="btn btn-outline-primary float-left" type="submit" onclick="return confirm('Tem certeza que deseja deletar?')">
                                        <i class="fas fa-trash-alt"></i>
                                    </button>
                                </form>
                                <a href="{{ route('selo.edit', $item->id)}}" class="btn btn-outline-primary"> <i class="fas fa-pen-alt"></i></a>
                                </td>
                              </tr>
                             @empty
